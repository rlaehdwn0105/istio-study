...
### Envoy를 이해할 때 놓치면 안 되는 관점

1주차에서는 아직 고급 xDS나 필터 체인을 깊게 볼 필요는 없다.  
대신 아래 관점은 분명히 잡아두는 편이 좋다.

1. Envoy는 애플리케이션 바깥에 있는 실행 계층이다.  
   따라서 애플리케이션 코드 변경 없이 공통 정책을 적용할 수 있다.

2. Envoy는 `요청이 어디로 갈지`만 정하는 것이 아니라,  
   `요청이 어떻게 실패하고, 어떻게 복구되고, 어떤 메타데이터를 남길지`까지 다룬다.

3. Istio를 쓴다는 것은 결국 `Envoy 집합을 제어 가능한 방식으로 운영한다`는 말과 거의 같다.

즉, 1주차에서 Envoy를 정확히 이해한다는 것은 단순 프록시 지식을 쌓는 것이 아니라,  
`Istio가 실제로 작동하는 현장`을 이해하는 것이다.
...
### 1주차 시점에서 Istiod를 너무 좁게 이해하면 안 되는 이유

입문 단계에서는 종종 "Istiod는 그냥 설정을 뿌리는 컴포넌트" 정도로 이해하고 넘어간다.  
물론 틀린 말은 아니지만, 그렇게만 이해하면 이후 보안과 관측성 주제를 따로따로 배우게 된다.

더 정확한 이해는 이렇다.

- Istiod는 메시 안의 서비스 관계를 이해한다
- 사용자가 선언한 정책을 프록시별 실행 설정으로 바꾼다
- 따라서 라우팅, 보안, 텔레메트리 모두 control plane의 해석과 배포를 거친다

즉, Istiod는 단순한 배포기가 아니라 `메시 정책을 실제 데이터 플레인으로 연결하는 핵심 제어면`이다.
...
### 사이드카 패턴을 이해할 때 중요한 운영 시사점

사이드카는 기능적으로는 매우 강력하다.  
하지만 운영자 관점에서 보면 다음 사실도 동시에 따라온다.

- 프록시 버전 관리가 필요하다
- 애플리케이션과 프록시를 함께 고려해야 한다
- 트래픽 이슈가 생기면 애플리케이션 로그만 봐서는 부족하다
- 파드 수가 많아질수록 프록시 리소스 비용도 누적된다

1주차에서는 이걸 깊게 다루지 않더라도, 최소한 "`사이드카는 무료 기능이 아니다`"라는 감각을 가져가는 것이 좋다.
...
### Bookinfo를 단순 데모가 아니라 학습 모델로 보는 법

Bookinfo는 화면이 보인다는 이유로 자주 "입문용 샘플"로만 소비된다.  
하지만 실제로는 다음 세 가지를 동시에 담고 있어서 학습 모델로 가치가 높다.

1. **서비스 호출 관계**  
   단순한 단일 서비스가 아니라, 사용자 요청이 여러 백엔드 호출로 나뉜다.

2. **버전 실험 가능성**  
   reviews 같은 서비스는 트래픽 분할과 점진 배포 실습에 적합하다.

3. **사용자 체감 가능성**  
   브라우저 화면 결과가 바뀌기 때문에 정책 변화가 추상적으로 끝나지 않는다.

즉, Bookinfo는 "눈에 보이는 서비스 메시 실험장"이라고 이해하는 편이 맞다.
...
### 1주차 실습을 통해 반드시 얻어야 하는 결과

실습의 결과는 단순히 "페이지가 열렸다"가 아니다.  
다음 설명이 가능해야 한다.

- 왜 Istio 설치가 애플리케이션 배포보다 먼저 오는가
- 왜 Gateway 적용이 마지막 단계인가
- 왜 파드에 `istio-proxy`가 붙는지
- 왜 Bookinfo가 이후 트래픽 제어 실습의 기반이 되는지

즉, 1주차 실습은 명령을 수행하는 과정이 아니라 `구조를 관찰하는 과정`이어야 한다.
...
자료를 읽을 때는 아래 세 질문을 반복하는 것이 좋다.

1. 이 문단은 어떤 문제를 설명하는가
2. 이 개념은 control plane 관점인가, data plane 관점인가
3. 이 내용은 1주차에서 끝나는가, 아니면 다음 주차로 이어지는가

이렇게 읽으면 정보가 한꺼번에 덮치지 않고, 주차별로 분리되어 정리된다.
...
추가로, 아래 두 질문에 답할 수 있으면 1주차 이해도가 꽤 높은 편이다.

- "Istio는 왜 애플리케이션 프레임워크 기능 확장이 아니라 별도 인프라 계층으로 설계되었는가?"
- "Bookinfo 없이도 개념은 배울 수 있는데, 굳이 샘플 앱을 쓰는 이유는 무엇인가?"
...
### 앞으로 같은 방식으로 정리할 때의 기준

1주차뿐 아니라 이후 주차도 같은 기준으로 정리하는 편이 좋다.

1. 먼저 공식 문서의 개념 정의를 기준선으로 삼는다
2. 그 다음 블로그에서 맥락과 한국어 설명을 보강한다
3. 버전 의존 명령은 반드시 다시 검수한다
4. 실습은 "명령 나열"보다 "왜 이 순서인가"를 설명한다
5. 마지막에는 다음 주차와 이어지는 질문을 남긴다
